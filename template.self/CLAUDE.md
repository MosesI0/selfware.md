...
### 兼容性适配
当 Agent 收到一个 `.self` 文件但当前环境不完全支持时（缺少依��、运行时版本不匹配等），**首次运行时应主动尝试适配**：安装缺失依赖、调整配置、补全运行时脚本，���其可运行。用户只需对自己的环境和需求负责，Agent 负责弥合差异。

### 需求冲突管理
每次收到新需求时，MUST 先搜索 `process/decisions/` 中的已有决策：
- **无冲突** → 将新需求作为��策记录写入 `process/decisions/`
- **有冲突** → 不得���默覆盖。MUST：
  1. 向用户明确提示冲突点
  2. 引导用户确认并细化需求
  3. 设计两个需求的共存方案，而非简单替换
